...
export default class WxUtils extends base {
  /**
   ...
   * 上传文件
   */
  static async uploadImage(filePath, formData) {
    const url = `${this.baseUrl}/images`
    const accessToken = wepy.getStorageSync('access_token')
    formData.plant = wepy.$instance.globalData.plantType
    const param = {
      url: url,
      method: 'POST',
      name: 'image',
